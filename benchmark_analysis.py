...
uuid_ordered_generation_times_ms = []

# for each file in dir, read CSV and get elapsed time for id generation
for f in os.listdir(os.path.join(os.getcwd(), 'reports', 'uuidbenchmark')):
    if f.endswith('.csv'):
        df = pd.read_csv(os.path.join(os.getcwd(), 'reports', 'uuidbenchmark', f))
        uuid_ordered_generation_times_ms.append(df.loc[df["Operation"] == "Generation"]["ElapsedTime"].values[0])
...
print(f"Mean: {np.mean(uuid_ordered_generation_times_ms)}")
print(f"Median: {np.median(uuid_ordered_generation_times_ms)}")
print(f"Standard deviation: {np.std(uuid_ordered_generation_times_ms)}")
print(f"Variance: {np.var(uuid_ordered_generation_times_ms)}")

print("T-TEST - ID GENERATION")
print(stats.ttest_ind(snowflake_generation_times_ms, uuid_ordered_generation_times_ms, equal_var=False))
print("\n =================================== \n")
